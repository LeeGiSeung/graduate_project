...
setence = ""
# 모델 추가하면 여기에 넣어야함
classfication_list = ["파리","가위","사과","풀다","눈","배","밤","가위","새"]
gl_driver = webdriver.Chrome(r"C:\Users\82103\Desktop\LSA\chromedriver-win64\chromedriver-win64\chromedriver.exe",options=chrome_options)
gl_ch_driver = r"C:\Users\82103\Desktop\LSA\chromedriver-win64\chromedriver-win64\chromedriver.exe"
gl_google_driver = r"C:\Users\82103\Desktop\LSA\chromedriver-win64\chromedriver-win64\chromedriver.exe"
# MongoDB 연결
...
def YesNo_classfication(text):
            # TF-IDF 모델 불러오기
    vectorizer = joblib.load(f'Model/긍정부정_tfidf_vectorizer.pkl')

    # 로지스틱 회귀 모델 불러오기
    lr_model = joblib.load(f'Model/긍정부정_logistic_regression_model.pkl')
    if isinstance(text, float):
        text = str(text)  # 부동 소수점을 문자열로 변환
    # 형태소 분석
    words = okt.pos(text, stem=True)
    # 명사와 동사 추출
    words = [word for word, pos in words if pos in ['Noun', 'Verb']]
    # 불용어 제거
    words = [word for word in words if word not in stopwords]
    new_sentence_preprocessed = ' '.join(words)
    new_sentence_vectorized = vectorizer.transform([new_sentence_preprocessed])

    predicted_class = lr_model.predict(new_sentence_vectorized)
...
# 동의어 분석
def Model_classfication(text,select_word,recommend):
    vectorizer = TfidfVectorizer()

        # TF-IDF 모델 불러오기
    vectorizer = joblib.load(f'Model/{select_word}_tfidf_vectorizer.pkl')

    # 로지스틱 회귀 모델 불러오기
    lr_model = joblib.load(f'Model/{select_word}_logistic_regression_model.pkl')
    ...
    if predicted_class[0] == 0:
        selected_value = zero[select_word]
        print(selected_value + select_word + recommend,"라고 판단했습니다.")
        return selected_value + select_word + recommend
    else:
        selected_value = one[select_word]
        print(selected_value + select_word + recommend,"라고 판단했습니다.")
        return selected_value + select_word + recommend
...
def process_and_store_data(sentence_input, user_input):
    ...
    filtered_words = [word for word in select_token if not word.isdigit()]
...
def classfication_Find_Answer(text):

    # 검색창에 '검색어' 검색

    filtered_keyword = text
    print()
    print("#########################")
    print("키워드 검색",text)
    print()
    ...
    blog_driver = webdriver.Chrome(gl_ch_driver, options=chrome_options)
    ...
    # chromedriver path input
        # option
    google_driver = webdriver.Chrome(gl_google_driver,options=chrome_options)
    ...
    # option
    driver = webdriver.Chrome(gl_ch_driver,options=chrome_options)
...
# 지식인(text), 구글 답변 검색
def Find_Answer(insert_keyword,key):
    import re
    ...
    # 검색창에 '검색어' 검색
    YesNo = YesNo_classfication(insert_keyword)
    global recommend

    recommend = YesNo
    filtered_keyword = re.sub(r'[^a-zA-Z가-힣\s]', '', str(keyword1))
    filtered_keyword = filtered_keyword + YesNo
    if(key == 0):
        print()
        print("##############################")
        print("유의어에 대해 검색합니다.")
        if any(word in filtered_keyword for word in classfication_list):
            print()
            print()
            print(filtered_keyword,"의 유의어에 대한 추천")
            for word in classfication_list:
                if word in filtered_keyword:
                    classfication_Find_Answer(Model_classfication(insert_keyword, word, recommend))
            print()
            print()
        else:
            print("유의어가 없습니다.")
    ...
    # 검색어 설정
    search_query = filtered_keyword# 원하는 검색어로 교체하세요
    ...
    google_url = 'https://www.google.com/search?q=' + quote_plus(filtered_keyword)
    # chromedriver path input
        # option
    google_driver = webdriver.Chrome(gl_google_driver,options=chrome_options)
...
def similarity(setntence_input):
    ...
    if sim_result_sorted:
        ...
        if(most_similar_pair["sim"]>0.7):
            ...
            print("\n\n")
            Find_Answer(most_similar_pair["ori"],1)
            print()
        else:
            print("유사한 문장이 없습니다.")
...
def Find_satisfatction_Answer(selected_topic):
    ...
    if any(word in filtered_keyword for word in classfication_list):
        ...
        for word in classfication_list:
               if word in filtered_keyword:
                    classfication_Find_Answer(Model_classfication(setence, word,recommend))
        print()
        print()
    else:
        print("유의어가 없습니다.")
    ...
    # 검색어 설정
    search_query = filtered_keyword + recommend  # 원하는 검색어로 교체하세요
    print("만족 x 결과 recommand 전역변수 확인용",search_query)
    # 검색 결과 가져오기
    search_response = youtube.search().list(
        q=search_query,
        type="video",
